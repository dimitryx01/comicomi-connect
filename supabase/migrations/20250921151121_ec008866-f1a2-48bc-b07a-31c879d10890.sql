...
CREATE TABLE public.postal_codes (
  ...
  area_name TEXT, -- neighborhood or area name within the city
  is_active BOOLEAN NOT NULL DEFAULT true,
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  UNIQUE(postal_code, city_id)
);

-- Migrate existing location data to new structure
WITH unique_cities AS (
  SELECT DISTINCT 
    ...
    array_agg(DISTINCT unnest(search_terms)) FILTER (WHERE search_terms IS NOT NULL) as all_search_terms
  FROM locations 
...
AS $function$
  WITH search_terms AS (
    SELECT 
      ...
      CASE 
        -- Exact match with municipality (highest relevance)
        WHEN LOWER(c.municipality) = LOWER(search_query) THEN 100
        -- Partial match with municipality
        ...
        -- Match with autonomous community
        ...
        WHEN EXISTS (
          SELECT 1 FROM unnest(c.search_terms) AS term 
...
-- Create RLS policies for postal codes (public read)
...
CREATE INDEX idx_cities_search_terms ON cities USING GIN(search_terms);
