-- Mejorar la función de búsqueda de ciudades para manejar búsquedas parciales y acentos
...
AS $function$
  WITH normalized_query AS (
    SELECT 
      ...
      LOWER(UNACCENT(TRIM(search_query))) as query_unaccent
  ),
  search_results AS (
    SELECT 
      l.id,
      ...
      CASE 
        ...
        WHEN LOWER(UNACCENT(l.municipality)) = nq.query_unaccent THEN 95
        
        -- Municipio empieza con la búsqueda
        WHEN LOWER(l.municipality) LIKE nq.query_lower || '%' THEN 90
        WHEN LOWER(UNACCENT(l.municipality)) LIKE nq.query_unaccent || '%' THEN 85
        
        -- Coincidencia parcial con municipio
        WHEN LOWER(l.municipality) LIKE '%' || nq.query_lower || '%' THEN 80
        WHEN LOWER(UNACCENT(l.municipality)) LIKE '%' || nq.query_unaccent || '%' THEN 75
        ...
        WHEN EXISTS (
          SELECT 1 FROM unnest(COALESCE(l.search_terms, ARRAY[]::text[])) AS term 
          WHERE LOWER(UNACCENT(term)) = nq.query_unaccent
        ) THEN 83
        WHEN EXISTS (
          SELECT 1 FROM unnest(COALESCE(l.search_terms, ARRAY[]::text[])) AS term 
        ...
        WHEN EXISTS (
          SELECT 1 FROM unnest(COALESCE(l.search_terms, ARRAY[]::text[])) AS term 
          WHERE LOWER(UNACCENT(term)) LIKE nq.query_unaccent || '%'
        ) THEN 73
        WHEN EXISTS (
          SELECT 1 FROM unnest(COALESCE(l.search_terms, ARRAY[]::text[])) AS term 
          WHERE LOWER(term) LIKE '%' || nq.query_lower || '%'
        ...
        WHEN LOWER(UNACCENT(l.province)) = nq.query_unaccent THEN 65
        WHEN LOWER(l.province) LIKE '%' || nq.query_lower || '%' THEN 60
        WHEN LOWER(UNACCENT(l.province)) LIKE '%' || nq.query_unaccent || '%' THEN 55
        
        -- Coincidencia con comunidad autónoma
        WHEN LOWER(l.autonomous_community) = nq.query_lower THEN 50
        WHEN LOWER(UNACCENT(l.autonomous_community)) = nq.query_unaccent THEN 45
        WHEN LOWER(l.autonomous_community) LIKE '%' || nq.query_lower || '%' THEN 40
        WHEN LOWER(UNACCENT(l.autonomous_community)) LIKE '%' || nq.query_unaccent || '%' THEN 35
...
-- Crear extensión UNACCENT si no existe (para manejar acentos)
CREATE EXTENSION IF NOT EXISTS unaccent;

-- Actualizar índices para mejor rendimiento
DROP INDEX IF EXISTS idx_locations_search_terms;
DROP INDEX IF EXISTS idx_locations_municipality_lower;
DROP INDEX IF EXISTS idx_locations_municipality_unaccent;

CREATE INDEX idx_locations_search_terms ON locations USING GIN(search_terms);
CREATE INDEX idx_locations_municipality_lower ON locations (LOWER(municipality));
CREATE INDEX idx_locations_municipality_unaccent ON locations (LOWER(UNACCENT(municipality))) WHERE municipality IS NOT NULL;