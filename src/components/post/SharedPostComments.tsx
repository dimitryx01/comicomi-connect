...
export const SharedPostComments = ({ 
  ...
  console.log('📝 SharedPostComments: Renderizando con:', {
    sharedPostId,
  ...
  const handleAddComment = async () => {
    if (newComment.trim() && currentUser) {
      console.log('➕ SharedPostComments: Agregando nuevo comentario');
      const success = await addComment(newComment);
      if (success) {
        setNewComment('');
        console.log('✅ SharedPostComments: Comentario agregado exitosamente');
      }
    }
  };
  ...
  const handleEditComment = useCallback((commentId: string, currentContent: string) => {
    console.log('✏️ SharedPostComments: Iniciando edición:', { commentId, currentContent: currentContent.substring(0, 50) + '...' });
  ...
  const handleSaveEdit = useCallback(async () => {
    if (!editingCommentId || !editingContent.trim()) {
      console.warn('⚠️ SharedPostComments: Intento de guardar sin ID o contenido');
      return;
    }

    console.log('💾 SharedPostComments: Guardando edición:', { editingCommentId, newContent: editingContent.substring(0, 50) + '...' });
    
    const success = await editComment(editingCommentId, editingContent);
    if (success) {
      console.log('✅ SharedPostComments: Edición guardada exitosamente');
      setEditingCommentId(null);
      setEditingContent('');
    } else {
      console.error('❌ SharedPostComments: Error al guardar edición');
    }
  }, [editingCommentId, editingContent, editComment]);
  ...
  const handleDeleteComment = useCallback(async (commentId: string) => {
    ...
    if (success) {
      console.log('✅ SharedPostComments: Comentario eliminado exitosamente');
    } else {
      console.error('❌ SharedPostComments: Error al eliminar comentario');
    }
  }, [deleteComment]);
          ...
          <div className="flex items-center space-x-2 mb-4">
            ...
            <div className="flex-1 flex items-center space-x-2">
              ...
              <Button 
                ...
                disabled={!newComment.trim() || commentsLoading || actionLoading}
              >
